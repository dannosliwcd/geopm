...
def do_power_limit_prologue():
    ...
    if node_power_limit_requested:
        ...
        # NOTE: if both a job power request and a node power request were made
        # by the user, then both resources are reconciled to the higher total
        # power of the two requests. That reconciliation happens in
        # do_power_limit_queuejob(), before reaching this block.
...
def do_power_limit_queuejob():
    ...
    user's requested power cap (if provided) to the administrator's allowed
    range or sets a preliminary job power resource request on a queued job so
    that the scheduler knows the minimum amount of power needed by the job.
    """
    server = pbs.server()
    ...
    if submitted_node_limit is not None:
    ...
    if (max_power_in_pbs_server is None and submitted_job_limit is None):
        # No job limit has been specified by the admin or by the user, so we have
        # nothing else to do.
        return
