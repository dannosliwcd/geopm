...
namespace geopm
{
    ...
    static const char CREATE_TABLES[] = "CREATE TABLE IF NOT EXISTS DefaultPolicies("
                                        " agent TEXT NOT NULL,"
    ...
    static std::vector<double> sqlite_results_to_policy_vector(sqlite3 *database, sqlite3_stmt *statement)
    {
        ...
        while ((sqlite_ret = sqlite3_step(statement)) == SQLITE_ROW) {
            ...
            auto value = sqlite3_column_type(statement, 1) == SQLITE_NULL ? NAN
    ...
    static UniqueStatement make_statement(sqlite3 *database, const char *statement)
    {
        ...
        int sqlite_ret = sqlite3_prepare_v2(database, statement, SQLITE_TERMINATED_STRING, &statement_raw_ptr,
        ...
        return UniqueStatement(statement_raw_ptr,
                               [](sqlite3_stmt *statement) { static_cast<void>(sqlite3_finalize(statement)); });
    }
    ...
    static int bind_value_to_statement(sqlite3_stmt *statement, int index, const std::string &text)
    {
        ...
        return sqlite3_bind_text(statement, index, text.c_str(), SQLITE_TERMINATED_STRING, UNUSED_STRING_DESTRUCTOR);
    }
    ...
    static std::vector<double> get_policy_from_best_policies(sqlite3 *database, const std::string &agent_name,
                                                             const std::string &profile_name)
    {
        static const char SELECT_BEST_POLICY[] = "SELECT offset,value "
                                                 "FROM BestPolicies "
    ...
    {
        static const char SELECT_default[] = "SELECT offset,value "
    ...
    PolicyStoreImp::~PolicyStoreImp()
    {
        ...
        if (ret != SQLITE_OK) {
            std::cerr << "Warning: <geopm> PolicyStore: Error while closing database. " << sqlite3_errstr(ret)
                      << std::endl;
        }
    }

    std::vector<double> PolicyStoreImp::get_best(const std::string &agent_name, const std::string &profile_name) const
    {
    ...
    void PolicyStoreImp::set_best(const std::string &agent_name, const std::string &profile_name,
                                  const std::vector<double> &policy)
    {
        ...
        begin_transaction_or_throw(m_database);
        {
            ...
            auto statement = make_statement(m_database, "DELETE FROM BestPolicies WHERE profile=?1 AND agent=?2;");
        ...
        for (size_t offset = 0; offset < policy.size(); ++offset) {
            auto statement = make_statement(m_database, "INSERT INTO BestPolicies "
    ...
    void PolicyStoreImp::set_default(const std::string &agent_name, const std::vector<double> &policy)
    {
        // check that the policy is valid for the agent
        size_t num_policy = Agent::num_policy(agent_name);
        if (policy.size() > num_policy) {
        ...
        begin_transaction_or_throw(m_database);
        {
            // Remove existing policy values for this record in case the new
            // policy does not explicitly overwrite all values.
            auto statement = make_statement(m_database, "DELETE FROM DefaultPolicies WHERE agent=?1;");
        ...
        for (size_t offset = 0; offset < policy.size(); ++offset) {
            auto statement = make_statement(m_database, "INSERT INTO DefaultPolicies "
