...
namespace geopm
{
    ...
    {
        static const std::map<int, std::string> event_names{{EVENT_REGION_ENTRY, "REGION_ENTRY"},
    ...
    {
        static const std::map<std::string, int> event_types{{"REGION_ENTRY", EVENT_REGION_ENTRY},
                                                            {"REGION_EXIT", EVENT_REGION_EXIT},
                                                            {"EPOCH_COUNT", EVENT_EPOCH_COUNT},
                                                            {"EVENT_SHORT_REGION", EVENT_SHORT_REGION}};
        auto it = event_types.find(event_name);
        if (it == event_types.end()) {
            throw geopm::Exception("invalid event type string: " + event_name, GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
        return it->second;
    }
    ...
    std::string hint_name(uint64_t hint)
    {
        static const std::string hint_names[] = {
            "UNSET",  "UNKNOWN",  "COMPUTE", "MEMORY",   "NETWORK", "IO",
            "SERIAL", "PARALLEL", "IGNORE",  "INACTIVE", "SPIN",
        };
        static_assert(sizeof(hint_names) / sizeof(hint_names[0]) == GEOPM_NUM_REGION_HINT,
    ...
    {
        static const std::pair<std::string, geopm_region_hint_e> hint_mapping[] = {
        ...
        static constexpr unsigned int HINT_MAP_SIZE = sizeof(hint_mapping) / sizeof(hint_mapping[0]);
        static_assert(HINT_MAP_SIZE == GEOPM_NUM_REGION_HINT, "hint_mapping mismatch with geopm_region_hint_e");
        static const std::map<std::string, uint64_t> result_map(hint_mapping, hint_mapping + HINT_MAP_SIZE);
