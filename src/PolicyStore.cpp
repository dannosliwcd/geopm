...
int geopm_policystore_get_best(const char *agent_name, const char *profile_name, size_t max_policy_vals,
                               double *policy_vals)
{
    int err = 0;
    if (!connected_store) {
        ...
        try {
            ...
            if (!policy_vals || best.size() > max_policy_vals) {
                ...
                std::fill_n(policy_vals + best.size(), max_policy_vals - best.size(), NAN);
            }
        }
        catch (...) {
            err = geopm::exception_handler(std::current_exception());
...
int geopm_policystore_set_best(const char *agent_name, const char *profile_name, size_t num_policy_vals,
