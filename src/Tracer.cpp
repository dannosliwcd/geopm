...
namespace geopm
{
    TracerImp::TracerImp(const std::string &start_time)
        : TracerImp(start_time, environment().trace(), hostname(), environment().do_trace(),
                    PlatformIOProf::platform_io(), platform_topo(), environment().trace_signals())
    {
    }

    TracerImp::TracerImp(const std::string &start_time, const std::string &file_path, const std::string &hostname,
                         bool do_trace, PlatformIO &platform_io, const PlatformTopo &platform_topo,
    ...
    {
        if (m_is_trace_enabled) {
            if (col_formats.size() != 0 && col_formats.size() != agent_cols.size()) {
                throw Exception("TracerImp::columns(): input vectors not of equal size", GEOPM_ERROR_INVALID,
                                __FILE__, __LINE__);
            }
            // default columns
            std::vector<struct m_request_s> base_columns(
                {{"TIME", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("TIME")},
                 {"EPOCH_COUNT", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("EPOCH_COUNT")},
                 {"REGION_HASH", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("REGION_HASH")},
                 {"REGION_HINT", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("REGION_HINT")},
                 {"REGION_PROGRESS", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("REGION_PROGRESS")},
                 {"CPU_ENERGY", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("CPU_ENERGY")},
                 {"DRAM_ENERGY", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("DRAM_ENERGY")},
                 {"CPU_POWER", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("CPU_POWER")},
                 {"DRAM_POWER", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("DRAM_POWER")},
                 {"CPU_FREQUENCY_STATUS", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("CPU_FREQUENCY_STATUS")},
                 {"CPU_CYCLES_THREAD", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("CPU_CYCLES_THREAD")},
                 {"CPU_CYCLES_REFERENCE", GEOPM_DOMAIN_BOARD, 0, m_platform_io.format_function("CPU_CYCLES_REFERENCE")},
...
#ifdef GEOPM_DEBUG
            if (env_sig.size() != env_dom.size() || env_sig.size() != env_form.size()) {
            ...
            for (const auto &col : base_columns) {
                m_column_idx.push_back(m_platform_io.push_signal(col.name, col.domain_type, col.domain_idx));
            ...
            for (size_t col_idx = 0; col_idx != num_col; ++col_idx) {
                std::function<std::string(double)> format = col_formats.size() ? col_formats.at(col_idx)
    ...
    void TracerImp::update(const std::vector<double> &agent_values)
    {
        if (m_is_trace_enabled) {
            ...
            if (m_column_idx.size() == 0) {
                throw Exception("TracerImp::update(): No columns added to trace.", GEOPM_ERROR_LOGIC,
                                __FILE__, __LINE__);
            }
