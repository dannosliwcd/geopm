...
static void on_ompt_event_parallel_begin(ompt_data_t *encountering_task_data,
                                         ...
                                         unsigned int requested_parallelism, int flags, const void *parallel_function)
{
    geopm::OMPT::ompt().region_enter(parallel_function);
}

static void on_ompt_event_parallel_end(ompt_data_t *parallel_data, ompt_data_t *encountering_task_data,
                                       int flags, const void *parallel_function)
{
    geopm::OMPT::ompt().region_exit(parallel_function);
}

static void on_ompt_event_work(ompt_work_t wstype, ompt_scope_endpoint_t endpoint, ompt_data_t *parallel_data,
                               ompt_data_t *task_data, uint64_t count, const void *parallel_function)
{
