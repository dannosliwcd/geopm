...
namespace geopm
{
    ...
    SSTFrequencyLimitDetector::SSTFrequencyLimitDetector(PlatformIO &platform_io, const PlatformTopo &platform_topo)
        ...
        , M_CPU_FREQUENCY_MAX(m_platform_io.read_signal("CPU_FREQUENCY_MAX_AVAIL", GEOPM_DOMAIN_BOARD, 0))
        , M_CPU_FREQUENCY_STICKER(m_platform_io.read_signal("CPU_FREQUENCY_STICKER", GEOPM_DOMAIN_BOARD, 0))
        , M_CPU_FREQUENCY_STEP(m_platform_io.read_signal("CPU_FREQUENCY_STEP", GEOPM_DOMAIN_BOARD, 0))
        , M_ALL_CORE_TURBO_FREQUENCY(
              m_platform_io.read_signal("MSR::TURBO_RATIO_LIMIT:MAX_RATIO_LIMIT_7", GEOPM_DOMAIN_BOARD, 0))
        , m_bucket_hp_cores{static_cast<unsigned int>(m_platform_io.read_signal("SST::HIGHPRIORITY_NCORES:0",
                                                                                GEOPM_DOMAIN_BOARD, 0)),
                            static_cast<unsigned int>(m_platform_io.read_signal("SST::HIGHPRIORITY_NCORES:1",
                                                                                GEOPM_DOMAIN_BOARD, 0)),
                            static_cast<unsigned int>(m_platform_io.read_signal("SST::HIGHPRIORITY_NCORES:2",
                                                                                GEOPM_DOMAIN_BOARD, 0))}
        , M_LOW_PRIORITY_SSE_FREQUENCY(
              m_platform_io.read_signal("SST::LOWPRIORITY_FREQUENCY:SSE", GEOPM_DOMAIN_BOARD, 0))
        , M_LOW_PRIORITY_AVX2_FREQUENCY(
              m_platform_io.read_signal("SST::LOWPRIORITY_FREQUENCY:AVX2", GEOPM_DOMAIN_BOARD, 0))
        , M_LOW_PRIORITY_AVX512_FREQUENCY(
              m_platform_io.read_signal("SST::LOWPRIORITY_FREQUENCY:AVX512", GEOPM_DOMAIN_BOARD, 0))
        , m_bucket_sse_frequency{m_platform_io.read_signal("SST::HIGHPRIORITY_FREQUENCY_SSE:0", GEOPM_DOMAIN_BOARD, 0),
                                 m_platform_io.read_signal("SST::HIGHPRIORITY_FREQUENCY_SSE:1", GEOPM_DOMAIN_BOARD, 0),
                                 m_platform_io.read_signal("SST::HIGHPRIORITY_FREQUENCY_SSE:2", GEOPM_DOMAIN_BOARD, 0)}
        , m_bucket_avx2_frequency{m_platform_io.read_signal("SST::HIGHPRIORITY_FREQUENCY_AVX2:0",
                                                            GEOPM_DOMAIN_BOARD, 0),
                                  m_platform_io.read_signal("SST::HIGHPRIORITY_FREQUENCY_AVX2:1",
                                                            GEOPM_DOMAIN_BOARD, 0),
                                  m_platform_io.read_signal("SST::HIGHPRIORITY_FREQUENCY_AVX2:2",
                                                            GEOPM_DOMAIN_BOARD, 0)}
        , m_bucket_avx512_frequency{m_platform_io.read_signal("SST::HIGHPRIORITY_FREQUENCY_AVX512:0",
                                                              GEOPM_DOMAIN_BOARD, 0),
                                    m_platform_io.read_signal("SST::HIGHPRIORITY_FREQUENCY_AVX512:1",
                                                              GEOPM_DOMAIN_BOARD, 0),
                                    m_platform_io.read_signal("SST::HIGHPRIORITY_FREQUENCY_AVX512:2",
                                                              GEOPM_DOMAIN_BOARD, 0)}
        , m_sse_hp_tradeoffs()
        , m_avx2_hp_tradeoffs()
        ...
        , m_core_lp_frequencies(m_core_count, M_CPU_FREQUENCY_STICKER)
    {
        for (size_t i = 0; i < m_core_count; ++i) {
            m_clos_association_signals.push_back(
                m_platform_io.push_signal("SST::COREPRIORITY:ASSOCIATION", GEOPM_DOMAIN_CORE, i));
            m_frequency_limit_signals.push_back(
                m_platform_io.push_signal("CPU_FREQUENCY_MAX_CONTROL", GEOPM_DOMAIN_CORE, i));
        }

        for (unsigned int i = 0; i < m_package_count; ++i) {
            m_sst_tf_enable_signals.push_back(
                m_platform_io.push_signal("SST::TURBO_ENABLE:ENABLE", GEOPM_DOMAIN_PACKAGE, i));
        ...
        for (size_t bucket = 0; bucket < m_bucket_hp_cores.size(); ++bucket) {
            m_sse_hp_tradeoffs.emplace_back(m_bucket_hp_cores[bucket], m_bucket_sse_frequency[bucket]);
            m_avx2_hp_tradeoffs.emplace_back(m_bucket_hp_cores[bucket], m_bucket_avx2_frequency[bucket]);
            m_avx512_hp_tradeoffs.emplace_back(m_bucket_hp_cores[bucket], m_bucket_avx512_frequency[bucket]);
        }
    }

    void SSTFrequencyLimitDetector::update_max_frequency_estimates(const std::vector<double> &observed_core_frequencies)
    {
        for (size_t package_idx = 0; package_idx < m_package_count; ++package_idx) {
            ...
            if (m_platform_io.sample(m_sst_tf_enable_signals[package_idx])) {
                auto hp_core_count = std::count_if(cores_in_package.begin(), cores_in_package.end(), [this](int idx) {
                ...
                const auto bucket_it = std::find_if(m_bucket_hp_cores.begin(), m_bucket_hp_cores.end(),
                ...
                for (const auto core_idx : cores_in_package) {
                    ...
                    if (observed_core_frequencies[core_idx] > avx2_freq || observed_core_frequencies[core_idx] >= sse_freq
                ...
                for (const auto core_idx : cores_in_package) {
                    m_core_frequency_limits[core_idx] = {// Single-element vector because this is the only
    ...
    double SSTFrequencyLimitDetector::get_core_low_priority_frequency(unsigned int core_idx) const
    {
        return m_core_lp_frequencies.at(core_idx);
    }

    bool SSTFrequencyLimitDetector::is_frequency_near_limit(double frequency, double limit)
    {
