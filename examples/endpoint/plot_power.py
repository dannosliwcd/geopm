...
for report_path in args.report_paths:
    ...
    for host in report_hosts:
        ...
        host_traces.append(df.set_index('TIMESTAMP').resample('20ms')[['CPU_POWER', 'CPU_POWER_LIMIT_CONTROL']].mean())
        hosts.append(host)

df = pd.concat(host_traces, keys=hosts, names=['Host'])
totals_df = df.groupby('TIMESTAMP').sum()
active_nodes_df = pd.DataFrame(active_node_changes, columns=['TIMESTAMP', 'Node Change']).sort_values('TIMESTAMP')
...
sns.lineplot(
    ax=ax_total,
    data=pd.melt(totals_df, ignore_index=False, var_name='Signal').reset_index(),
    x='TIMESTAMP',
    y='value',
    style='Signal')
...
sns.lineplot(
    ax=ax_parts,
    data=pd.melt(df, ignore_index=False, var_name='Signal').reset_index(),
    x='TIMESTAMP',
    y='value',
    hue='Host',
    style='Signal')
...
ax_nodes.step(x=active_nodes_df['TIMESTAMP'].values, y=active_nodes_df['Active Nodes'].values, where='post')
