...
void FrequencyBalancerAgentTest::SetUp()
{
    ...
    m_agent = geopm::make_unique<FrequencyBalancerAgent>(
            m_platform_io, m_platform_topo, m_power_governor, m_frequency_governor, m_sst_clos_governor, m_frequency_time_balancer);
}
...
TEST_F(FrequencyBalancerAgentTest, adjust_new_epoch)
{
    ...
    EXPECT_CALL(*m_frequency_time_balancer, balance_frequencies_by_time(_, _, _, _))
    ...
    EXPECT_CALL(*m_frequency_time_balancer, get_cutoff_frequency(_))
