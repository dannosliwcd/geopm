...
namespace geopm
{
    void SharedMemory::cleanup_shmem(void)
    {
        ...
        if (err == 0) {
            for (size_t path_idx = 0; path_idx != globbuf.gl_pathc; ++path_idx) {
                ...
                if (unlink_err != 0) {
#ifdef GEOPM_DEBUG
                    std::cerr << "Warning: <geopm> SharedMemory:cleanup_shmem(): unable to unlink file: "
                              << globbuf.gl_pathv[path_idx] << "\n";
#endif
                }
            }
        }
    ...
    static void setup_mutex(pthread_mutex_t *lock)
    {
        ...
        if (err) {
            throw Exception("SharedMemory::setup_mutex(): pthread mutex initialization", GEOPM_ERROR_RUNTIME,
                            __FILE__, __LINE__);
        }
    ...
    {
        if (!size) {
            throw Exception("SharedMemoryImp: Cannot create shared memory region of zero size",
        ...
        int shm_id = 0;
        if (is_secure) {
            shm_id = open(m_shm_path.c_str(), O_RDWR | O_CREAT | O_EXCL, S_IRUSR | S_IWUSR);
        }
        else {
        ...
        if (m_ptr == MAP_FAILED) {
            ...
            (void)::unlink(m_shm_path.c_str());
            (void)umask(old_mask);
            throw Exception("SharedMemoryImp: Could not mmap shared memory region",
                            errno ? errno : GEOPM_ERROR_RUNTIME, __FILE__, __LINE__);
        }
        err = close(shm_id);
        if (err) {
            (void)umask(old_mask);
            throw Exception("SharedMemoryImp: Could not close shared memory file",
                            errno ? errno : GEOPM_ERROR_RUNTIME, __FILE__, __LINE__);
        }
        umask(old_mask);
    ...
    void SharedMemoryImp::attach_memory_region(const std::string &shm_key, unsigned int timeout)
    {
        ...
        if (!timeout) {
            ...
            if (!m_size) {
                (void)close(shm_id);
                throw Exception("SharedMemoryImp: Opened shared memory region, but it is zero length",
                                errno ? errno : GEOPM_ERROR_RUNTIME, __FILE__, __LINE__);
            }
    ...
    void SharedMemoryImp::chown(const unsigned int uid, const unsigned int gid) const
    {
        ...
        if (m_is_linked) {
            ...
            throw Exception("SharedMemoryImp: Cannot chown shm that has been unlinked.", GEOPM_ERROR_RUNTIME,
                            __FILE__, __LINE__);
        }
        ...
        if (err) {
            (void)close(shm_id);
            std::ostringstream ex_str;
            ex_str << "SharedMemoryImp: Could not chown shmem with key (" << m_shm_key << ") to UID ("
    ...
    {
        std::string path;
        if (key.length() > 1 && key[0] == '/' && key.find('/', 1) == std::string::npos) {
            // shmem key

            // pam_systemd/logind enabled?
            std::string usr_run_dir = "/run/user/" + std::to_string(getuid());
