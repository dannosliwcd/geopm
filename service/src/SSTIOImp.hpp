...
namespace geopm
{
    ...
    class SSTIOImp : public SSTIO
    {
        public:
            ...
            int add_mbox_read(uint32_t cpu_index, uint16_t command, uint16_t subcommand,
                              uint32_t subcommand_arg) override;
            int add_mbox_write(uint32_t cpu_index, uint16_t command, uint16_t subcommand,
                               uint32_t interface_parameter, uint16_t read_subcommand,
                               uint32_t read_interface_parameter, uint32_t read_mask) override;
            int add_mmio_read(uint32_t cpu_index, uint16_t register_offset) override;
            int add_mmio_write(uint32_t cpu_index, uint16_t register_offset, uint32_t register_value,
            ...
            uint32_t read_mbox_once(uint32_t cpu_index, uint16_t command, uint16_t subcommand,
                                    uint32_t subcommand_arg) override;
            void write_mbox_once(uint32_t cpu_index, uint16_t command, uint16_t subcommand,
                                 uint32_t interface_parameter, uint16_t read_subcommand,
                                 uint32_t read_interface_parameter, uint32_t read_mask, uint64_t write_value,
                                 uint64_t write_mask) override;
            uint32_t read_mmio_once(uint32_t cpu_index, uint16_t register_offset) override;
            void write_mmio_once(uint32_t cpu_index, uint16_t register_offset, uint32_t register_value,
                                 uint32_t read_mask, uint64_t write_value, uint64_t write_mask) override;
        ...
        private:
            enum message_type_e { MBOX, MMIO };

            template<typename OuterStruct>
            using InnerStruct = typename std::remove_all_extents<decltype(OuterStruct::interfaces)>::type;
...

            {
                ...
                while (handled_commands < commands.size()) {
                    size_t batch_size = std::min(static_cast<size_t>(m_batch_command_limit),
                    ...
                    outer_structs.emplace_back(
                        reinterpret_cast<OuterStruct *>(new char[sizeof(OuterStruct::num_entries)
                                                                 + sizeof(InnerStruct<OuterStruct>) * commands.size()]),
                        [](OuterStruct *outer_struct) { delete[] reinterpret_cast<char *>(outer_struct); });
                    outer_structs.back()->num_entries = batch_size;
                    std::copy(commands.data() + handled_commands,
                              commands.data() + handled_commands + batch_size, outer_structs.back()->interfaces);
