...
namespace geopm
{
    TimeIOGroup::TimeIOGroup()
        ...
        , m_time_curr(NAN)
        , m_valid_signal_name{plugin_name() + "::ELAPSED", "TIME"}
    {
    }
    ...
    {
        if (!is_valid_signal(signal_name)) {
            throw Exception("TimeIOGroup::push_signal(): signal_name " + signal_name + " not valid for TimeIOGroup",
    ...
    {
        if (!m_is_signal_pushed) {
            throw Exception("TimeIOGroup::sample(): signal has not been pushed", GEOPM_ERROR_INVALID,
                            __FILE__, __LINE__);
        }
        if (!m_is_batch_read) {
            throw Exception("TimeIOGroup::sample(): signal has not been read", GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
        if (batch_idx != 0) {
            throw Exception("TimeIOGroup::sample(): batch_idx out of range", GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
        return m_time_curr;
    }
    ...
    {
        if (!is_valid_signal(signal_name)) {
            throw Exception("TimeIOGroup:read_signal(): " + signal_name + "not valid for TimeIOGroup",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
        if (domain_type != GEOPM_DOMAIN_CPU) {
    ...
    {
        if (!is_valid_signal(signal_name)) {
            throw Exception("TimeIOGroup::agg_function(): " + signal_name + "not valid for TimeIOGroup",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
    ...
    {
        if (!is_valid_signal(signal_name)) {
            throw Exception("TimeIOGroup::format_function(): " + signal_name + "not valid for TimeIOGroup",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
    ...
    {
        if (!is_valid_signal(signal_name)) {
            throw Exception("TimeIOGroup::signal_description(): " + signal_name + "not valid for TimeIOGroup",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
    ...
    {
        if (!is_valid_signal(signal_name)) {
            throw Exception("TimeIOGroup::signal_behavior(): " + signal_name + "not valid for TimeIOGroup",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
