...
namespace geopm
{
        ...
        , m_num_gpu_subdevice(0)
    {
        ...
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": LevelZero Driver failed to initialize.",
                        __LINE__);
        ...
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": LevelZero Driver enumeration failed.", __LINE__);
        m_levelzero_driver.resize(num_driver);
        ze_result = zeDriverGet(&num_driver, m_levelzero_driver.data());
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": LevelZero Driver acquisition failed.", __LINE__);
        ...
        for (unsigned int driver = 0; driver < num_driver; driver++) {
            ...
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": LevelZero Device enumeration failed.",
                            __LINE__);
            ...
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": LevelZero Device acquisition failed.",
                            __LINE__);
            ...
            for (unsigned int device_idx = 0; device_idx < num_device; ++device_idx) {
                ...
                check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                                "LevelZero::" + std::string(__func__) + ": failed to get device properties.",
                                __LINE__);
                ...
                check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                                "LevelZero::" + std::string(__func__) + ": LevelZero Sub-Device enumeration failed.",
                                __LINE__);

                std::vector<zes_device_handle_t> subdevice_handle(num_subdevice);
                ze_result = zeDeviceGetSubDevices(device_handle.at(device_idx), &num_subdevice,
                                                  subdevice_handle.data());
                check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                                "LevelZero::" + std::string(__func__) + ": LevelZero Sub-Device acquisition failed.",
                                __LINE__);
#ifdef GEOPM_DEBUG
                if (num_subdevice == 0) {
                    std::cerr << "LevelZero::" << std::string(__func__)
                              << ": GEOPM Requires at least one subdevice. "
                                 ...
                                 "setting.  Forcing device to act as sub-device"
                              << std::endl;
                }
#endif
            ...
            if (m_num_gpu != 0 && m_num_gpu_subdevice % m_num_gpu != 0) {
                throw Exception("LevelZero::" + std::string(__func__) + ": GEOPM Requires the number of subdevices to be"
            ...
            for (unsigned int idx = 1; idx < m_devices.size(); ++idx) {
                if (m_devices.at(idx).m_num_subdevice != m_devices.at(idx - 1).m_num_subdevice) {
                    throw Exception("LevelZero::" + std::string(__func__)
                                        + ": GEOPM Requires the number of subdevices to be" + " the same on all devices. "
                                        + " Please check ZE_AFFINITY_MASK environment variable settings",
                                    GEOPM_ERROR_INVALID, __FILE__, __LINE__);
                }
            }
    ...
    void LevelZeroImp::frequency_domain_cache(unsigned int device_idx)
    {
        ...
        if (ze_result == ZE_RESULT_ERROR_UNSUPPORTED_FEATURE) {
            ...
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": Sysman failed to get number of domains.",
                            __LINE__);
            // make temp var
            std::vector<zes_freq_handle_t> freq_domain(num_domain);

            ze_result = zesDeviceEnumFrequencyDomains(m_devices.at(device_idx).device_handle, &num_domain,
                                                      freq_domain.data());
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": Sysman failed to get domain handles.",
                            __LINE__);
            ...
            for (auto handle : freq_domain) {
                ...
                check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                                "LevelZero::" + std::string(__func__) + ": Sysman failed to get domain properties.",
                                __LINE__);
                ...
                if (property.onSubdevice == 0 && m_devices.at(device_idx).m_num_subdevice != 0) {
                    ...
                    if (property.type == ZES_FREQ_DOMAIN_GPU) {
                        m_devices.at(device_idx).subdevice.freq_domain.at(geopm::LevelZero::M_DOMAIN_COMPUTE).push_back(handle);
                    }
                    else if (property.type == ZES_FREQ_DOMAIN_MEMORY) {
                        m_devices.at(device_idx).subdevice.freq_domain.at(geopm::LevelZero::M_DOMAIN_MEMORY).push_back(handle);
                    }
                }
            }
        }
    ...
    void LevelZeroImp::power_domain_cache(unsigned int device_idx)
    {
        ...
        if (ze_result == ZE_RESULT_ERROR_UNSUPPORTED_FEATURE) {
            ...
            ze_result = zesDeviceEnumPowerDomains(m_devices.at(device_idx).device_handle, &num_domain,
                                                  power_domain.data());
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": Sysman failed to get domain handle(s).",
                            __LINE__);
            ...
            for (auto handle : power_domain) {
                ...
                check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                                "LevelZero::" + std::string(__func__) + ": Sysman failed to get domain power properties",
                                __LINE__);
            ...
            if (num_device_power_domain != 1) {
                throw Exception("LevelZero::" + std::string(__func__) + ": GEOPM requires one and only one device "
                                    + "level power domain (detected: " + std::to_string(num_device_power_domain) + ").",
                                GEOPM_ERROR_INVALID, __FILE__, __LINE__);
            }

            if (num_subdevice_power_domain > m_devices.at(device_idx).m_num_subdevice) {
                throw Exception("LevelZero::" + std::string(__func__) + ": Number of subdevice power domains ("
                                    + std::to_string(num_device_power_domain) + ") exceeds the number of subdevices ("
            ...
            m_devices.at(device_idx)
                .subdevice.cached_energy_timestamp.resize(m_devices.at(device_idx).subdevice.power_domain.size());
        }
    }
    ...
    void LevelZeroImp::perf_domain_cache(unsigned int device_idx)
    {
        ...
        num_domain = 0;
        ze_result = zesDeviceEnumPerformanceFactorDomains(m_devices.at(device_idx).device_handle, &num_domain, nullptr);
        if (ze_result != ZE_RESULT_ERROR_UNSUPPORTED_FEATURE) {
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": Sysman failed to get number of domains",
                            __LINE__);
            ...
            ze_result = zesDeviceEnumPerformanceFactorDomains(m_devices.at(device_idx).device_handle,
            ...
            for (auto handle : perf_domain) {
                ...
                if (property.onSubdevice != 0) {
                    if (property.engines == ZES_ENGINE_TYPE_FLAG_COMPUTE) {
                        m_devices.at(device_idx).subdevice.perf_domain.at(geopm::LevelZero::M_DOMAIN_COMPUTE).push_back(handle);
                    }
#ifdef GEOPM_DEBUG
                    else {
    ...
    void LevelZeroImp::engine_domain_cache(unsigned int device_idx)
    {
        ...
        if (ze_result == ZE_RESULT_ERROR_UNSUPPORTED_FEATURE) {
            ...
            ze_result = zesDeviceEnumEngineGroups(m_devices.at(device_idx).device_handle, &num_domain,
                                                  engine_domain.data());
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": Sysman failed to get number of domains",
                            __LINE__);
            ...
            for (auto handle : engine_domain) {
                ...
                check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                                "LevelZero::" + std::string(__func__) + ": Sysman failed to get domain engine properties",
                                __LINE__);
                ...
                if (property.onSubdevice == 0 && m_devices.at(device_idx).m_num_subdevice != 0) {
                    ...
                    if (property.type == ZES_ENGINE_GROUP_ALL) {
                        m_devices.at(device_idx).subdevice.engine_domain.at(geopm::LevelZero::M_DOMAIN_ALL).push_back(handle);
                        m_devices.at(device_idx).subdevice.cached_timestamp.at(geopm::LevelZero::M_DOMAIN_ALL).push_back(0);
                    }
                    ...
                    //      aggregate the signals in that case
                        ...
                        m_devices.at(device_idx).subdevice.engine_domain.at(geopm::LevelZero::M_DOMAIN_MEMORY).push_back(handle);
                        m_devices.at(device_idx)
...
#ifdef GEOPM_DEBUG
            if (num_domain != 0
                && m_devices.at(device_idx).subdevice.engine_domain.at(geopm::LevelZero::M_DOMAIN_COMPUTE).size()
                       == 0) {
                std::cerr << "Warning: <geopm> LevelZero: Engine domain detection "
                             "did not find ZES_ENGINE_GROUP_COMPUTE_ALL.\n";
            }
            if (num_domain != 0
                && m_devices.at(device_idx).subdevice.engine_domain.at(geopm::LevelZero::M_DOMAIN_MEMORY).size() == 0) {
                std::cerr << "Warning: <geopm> LevelZero: Engine domain detection "
    ...
    void LevelZeroImp::temperature_domain_cache(unsigned int device_idx)
    {
        ...
        // Cache frequency domains
        ze_result = zesDeviceEnumTemperatureSensors(m_devices.at(device_idx).device_handle, &num_domain, nullptr);
        if (ze_result == ZE_RESULT_ERROR_UNSUPPORTED_FEATURE) {
#ifdef GEOPM_DEBUG
            ...
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": Sysman failed to get number of temperature domains.",
                            __LINE__);
            // make temp var
            std::vector<zes_temp_handle_t> temp_domain(num_domain);

            ze_result = zesDeviceEnumTemperatureSensors(m_devices.at(device_idx).device_handle, &num_domain,
                                                        temp_domain.data());
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": Sysman failed to get temperature domain handles.",
                            __LINE__);
            ...
            for (auto handle : temp_domain) {
                ...
                check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                                "LevelZero::" + std::string(__func__)
                                    + ": Sysman failed to get temperature domain properties.",
                                __LINE__);
                ...
                if (property.onSubdevice == 0) {
                    ...
                    if (property.type == ZES_TEMP_SENSORS_GPU) {
                        m_devices.at(device_idx)
                            .subdevice.temp_domain_max.at(geopm::LevelZero::M_DOMAIN_COMPUTE)
                            .push_back(handle);
                    }
                    else if (property.type == ZES_TEMP_SENSORS_MEMORY) {
                        m_devices.at(device_idx)
                            .subdevice.temp_domain_max.at(geopm::LevelZero::M_DOMAIN_MEMORY)
                            .push_back(handle);
                    }
                    else if (property.type == ZES_TEMP_SENSORS_GLOBAL) {
                        m_devices.at(device_idx).subdevice.temp_domain_max.at(geopm::LevelZero::M_DOMAIN_ALL).push_back(handle);
                    }
                }
            }
        }
    ...
    int LevelZeroImp::num_gpu(int domain_type) const
    {
        ...
        switch (domain_type) {
            ...
            case GEOPM_DOMAIN_GPU_CHIP:
                ...
                break;
            default:
                throw Exception("LevelZero::" + std::string(__func__) + ": domain type "
                                    + std::to_string(domain_type) + " is not supported.",
                                GEOPM_ERROR_INVALID, __FILE__, __LINE__);
                break;
        }
        return result;
    ...
    int LevelZeroImp::power_domain_count(int geopm_domain, unsigned int l0_device_idx, int l0_domain) const
    {
        int count = 0;
    ...
    double LevelZeroImp::performance_factor(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
        double result = NAN;
        ...
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": Sysman failed to get performance factor values",
                        __LINE__);

        return result;
    }

    double LevelZeroImp::frequency_status(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
        return frequency_status_helper(l0_device_idx, l0_domain, l0_domain_idx).actual;
    }

    double LevelZeroImp::frequency_efficient(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
        return frequency_status_helper(l0_device_idx, l0_domain, l0_domain_idx).efficient;
    }

    uint32_t LevelZeroImp::frequency_throttle_reasons(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
                                                                      ...
                                                                      int l0_domain, int l0_domain_idx) const
    {
        ...
        zes_freq_handle_t handle = m_devices.at(l0_device_idx).subdevice.freq_domain.at(l0_domain).at(l0_domain_idx);
        zes_freq_state_t state = {};
        ze_result = zesFrequencyGetState(handle, &state);
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": Sysman failed to get frequency state", __LINE__);
    ...
    double LevelZeroImp::frequency_min(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
        return frequency_min_max(l0_device_idx, l0_domain, l0_domain_idx).first;
    }

    double LevelZeroImp::frequency_max(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
        return frequency_min_max(l0_device_idx, l0_domain, l0_domain_idx).second;
    }

    std::pair<double, double> LevelZeroImp::frequency_min_max(unsigned int l0_device_idx, int l0_domain,
    ...
    std::vector<double> LevelZeroImp::frequency_supported(unsigned int l0_device_idx, int l0_domain,
                                                          int l0_domain_idx) const
    {
        ze_result_t ze_result;
        zes_freq_handle_t handle = m_devices.at(l0_device_idx).subdevice.freq_domain.at(l0_domain).at(l0_domain_idx);
        ...
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": Sysman failed to get supported frequency count.",
                        __LINE__);

        std::vector<double> result(num_freq);
        ze_result = zesFrequencyGetAvailableClocks(handle, &num_freq, result.data());
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": Sysman failed to get supported frequency list.",
                        __LINE__);

        return result;
    }

    std::pair<double, double> LevelZeroImp::frequency_range(unsigned int l0_device_idx, int l0_domain,
                                                            int l0_domain_idx) const
    {
        ze_result_t ze_result;
        zes_freq_handle_t handle = m_devices.at(l0_device_idx).subdevice.freq_domain.at(l0_domain).at(l0_domain_idx);
        ...
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": Sysman failed to get frequency range.", __LINE__);
        return {range.min, range.max};
    }

    double LevelZeroImp::temperature_max(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
        ze_result_t ze_result;
        double result = NAN;

        zes_temp_handle_t handle = m_devices.at(l0_device_idx).subdevice.temp_domain_max.at(l0_domain).at(l0_domain_idx);

        ze_result = zesTemperatureGetState(handle, &result);

        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": Sysman failed to get temperature.", __LINE__);

        return result;
    }

    uint64_t LevelZeroImp::active_time_timestamp(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
        return m_devices.at(l0_device_idx).subdevice.cached_timestamp.at(l0_domain).at(l0_domain_idx);
    }

    uint64_t LevelZeroImp::active_time(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
        return active_time_pair(l0_device_idx, l0_domain, l0_domain_idx).first;
    }

    std::pair<uint64_t, uint64_t> LevelZeroImp::active_time_pair(unsigned int l0_device_idx, int l0_domain,
                                                                 int l0_domain_idx) const
    {
        ze_result_t ze_result;
        ...
        zes_engine_handle_t handle = m_devices.at(l0_device_idx).subdevice.engine_domain.at(l0_domain).at(l0_domain_idx);
        ze_result = zesEngineGetActivity(handle, &stats);
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": Sysman failed to get engine group activity.",
                        __LINE__);
        result_active = stats.activeTime;
        result_timestamp = stats.timestamp;
        m_devices.at(l0_device_idx).subdevice.cached_timestamp.at(l0_domain).at(l0_domain_idx) = result_timestamp;

        return {result_active, result_timestamp};
    }

    uint64_t LevelZeroImp::energy_timestamp(int geopm_domain, unsigned int l0_device_idx, int l0_domain,
                                            int l0_domain_idx) const
    {
    ...
    uint64_t LevelZeroImp::energy(int geopm_domain, unsigned int l0_device_idx, int l0_domain, int l0_domain_idx) const
    {
        return energy_pair(geopm_domain, l0_device_idx, l0_domain_idx).first;
    }

    std::pair<uint64_t, uint64_t> LevelZeroImp::energy_pair(int geopm_domain, unsigned int l0_device_idx,
                                                            int l0_domain_idx) const
    {
        ze_result_t ze_result;
        ...
        if (geopm_domain == GEOPM_DOMAIN_GPU
            ...
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZero::" + std::string(__func__) + ": Sysman failed to get energy_counter values",
                            __LINE__);
    ...
    LevelZeroImp::m_power_limit_s LevelZeroImp::power_limit_default(unsigned int l0_device_idx) const
    {
        ...
        if (m_devices.at(l0_device_idx).num_device_power_domain == 1) {
            ...
            ze_result = zesPowerGetProperties(handle, &property);
            check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                            "LevelZeroDevicePool::" + std::string(__func__) + ": Sysman failed to get domain power properties",
                            __LINE__);
    ...
    void LevelZeroImp::frequency_control(unsigned int l0_device_idx, int l0_domain, int l0_domain_idx,
                                         double range_min, double range_max) const
    {
        ...
        if (property.canControl == 0) {
            throw Exception("LevelZero::" + std::string(__func__) + ": Attempted to set frequency "
                                + "for non controllable domain",
                            GEOPM_ERROR_INVALID, __FILE__, __LINE__);
        }
        ze_result = zesFrequencySetRange(handle, &range);
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": Sysman failed to set frequency.", __LINE__);
    }
                                                  ...
                                                  int l0_domain_idx, double setting) const
    {
        ...
        check_ze_result(ze_result, GEOPM_ERROR_RUNTIME,
                        "LevelZero::" + std::string(__func__) + ": Sysman failed to set performance factor values",
                        __LINE__);
    }

    void LevelZeroImp::check_ze_result(ze_result_t ze_result, int error, std::string message, int line) const
    {
        ...
        if (ze_result != ZE_RESULT_SUCCESS) {
            throw Exception(message + " Level Zero Error: " + error_string, error, __FILE__, line);
        }
    }
}
