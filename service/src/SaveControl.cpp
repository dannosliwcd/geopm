...
namespace geopm
{
    std::unique_ptr<SaveControl> SaveControl::make_unique(const std::vector<m_setting_s> &settings)
    {
        return geopm::make_unique<SaveControlImp>(settings);
    }

    std::unique_ptr<SaveControl> SaveControl::make_unique(const std::string &json_string)
    {
        return geopm::make_unique<SaveControlImp>(json_string);
    }

    std::unique_ptr<SaveControl> SaveControl::make_unique(IOGroup &io_group)
    {
    ...
    std::vector<SaveControl::m_setting_s> SaveControlImp::settings(const std::string &json_string)
    {
        ...
        for (const auto &jss : root.array_items()) {
            ...
            std::vector<std::string> required_keys = {"name", "domain_type", "domain_idx", "setting"};
            ...
            for (const auto &rk : required_keys) {
                if (jss_map.count(rk) == 0) {
                    throw Exception("SaveControlImp::settings(): Invalid settings object JSON, missing a required field: \""
                                        + rk + "\"",
                                    GEOPM_ERROR_INVALID, __FILE__, __LINE__);
                }
            }
            result.push_back(m_setting_s{jss["name"].string_value(), jss["domain_type"].int_value(),
                                         jss["domain_idx"].int_value(), jss["setting"].number_value()});
        }
        return result;
    }
    ...
    {
        for (const auto &ss : settings()) {
            io_group.write_control(ss.name, ss.domain_type, ss.domain_idx, ss.setting);
        }
    }

    std::vector<SaveControl::m_setting_s> SaveControlImp::settings(IOGroup &io_group, const PlatformTopo &topo)
    {
        std::vector<m_setting_s> result;
        ...
        for (const auto &name : io_group.control_names()) {
            if (string_begins_with(name, prefix)) {
                ...
                for (int dom_idx = 0; dom_idx != num_dom; ++dom_idx) {
                    double setting = io_group.read_signal(name, dom_type, dom_idx);
                    result.push_back({name, dom_type, dom_idx, setting});
                }
            }
        }
        return result;
