    ...
    class SSTIOGroup : public IOGroup
    {
        ...
        private:
            struct sst_signal_mailbox_field_s {
                    ...
                    sst_signal_mailbox_field_s(uint32_t request_data, uint32_t begin_bit, uint32_t end_bit,
                                               double multiplier, int units, const std::string &description,
                                               m_signal_behavior_e behavior)
                        : sst_signal_mailbox_field_s(request_data, begin_bit, end_bit, multiplier, units,
                                                     description, behavior, Agg::expect_same)
                    {
                    }
            ...
            struct sst_control_mailbox_field_s {
                    sst_control_mailbox_field_s(uint32_t write_data, uint32_t begin_bit, uint32_t end_bit,
                                                int units, const std::string &description)
                        : sst_control_mailbox_field_s(write_data, begin_bit, end_bit, units, description, Agg::expect_same)
                    {
                    }
            ...
            struct sst_control_mailbox_raw_s {
                    ...
                    //! @param subcommand Subtype of the given command
                    //! @param fields Subfields of the mailbox
                    sst_control_mailbox_raw_s(SSTMailboxCommand command, uint16_t subcommand, uint32_t write_param,
            ...
            struct sst_control_mmio_field_s {
                    sst_control_mmio_field_s(uint32_t begin_bit, uint32_t end_bit, double multiplier,
                                             int units, const std::string &description)
                        : sst_control_mmio_field_s(begin_bit, end_bit, multiplier, units, description, Agg::expect_same)
                    {
                    }
            ...
            struct sst_signal_mmio_field_s {
                    sst_signal_mmio_field_s(uint32_t write_value, uint32_t begin_bit, uint32_t end_bit,
                                            double multiplier, int units, const std::string &description,
                                            m_signal_behavior_e behavior)
                        : sst_signal_mmio_field_s(write_value, begin_bit, end_bit, multiplier, units,
                                                  description, behavior, Agg::expect_same)
                    {
                    }
            ...
            void add_mbox_signals(const std::string &raw_name, SSTMailboxCommand command, uint16_t subcommand,
                                  const std::map<std::string, sst_signal_mailbox_field_s> &fields);
            void add_mbox_controls(const std::string &raw_name, SSTMailboxCommand command,
                                   uint16_t subcommand, uint32_t write_param,
                                   const std::map<std::string, sst_control_mailbox_field_s> &fields,
                                   uint16_t read_subcommand, uint32_t read_request_data, uint32_t read_mask);

            void add_mmio_signals(const std::string &raw_name, int domain_type, uint32_t register_offset,
                                  const std::map<std::string, sst_signal_mmio_field_s> &fields);
            void add_mmio_controls(const std::string &raw_name, int domain_type, uint32_t register_offset,
                                   const std::map<std::string, sst_control_mmio_field_s> &fields,
                                   uint32_t read_mask);
