...
namespace geopm
{
    ...
    class PlatformTopoImp : public PlatformTopo
    {
        ...
        private:
            ...
            std::set<int> domain_cpus(int domain_type, int domain_idx) const;

            void lscpu(std::map<std::string, std::string> &lscpu_map);
            void parse_lscpu(const std::map<std::string, std::string> &lscpu_map, int &num_package,
            ...
            std::vector<std::set<int> > parse_lscpu_gpu(const std::map<std::string, std::string> &lscpu_map,
                                                        int domain_type);
