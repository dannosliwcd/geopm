...
namespace geopm
{
    ...
    class SSTControl : public geopm::Control
    {
        public:
            ...
            ///             the write mask.
            SSTControl(std::shared_ptr<SSTIO> sstio, ControlType control_type, int cpu_idx, uint32_t command,
                       uint32_t subcommand, uint32_t interface_parameter, uint32_t write_value,
                       uint32_t begin_bit, uint32_t end_bit, double scale, uint32_t rmw_subcommand,
            ...
            void set_write_dependency(uint64_t trigger_value, std::weak_ptr<geopm::Control> dependency,
