...
namespace geopm
{
        ...
        : m_nvml_device_pool(device_pool)
    {
        ...
        if (num_gpu == 0) {
            ...
            /// @todo: As an optimization this may be replaceable with CPU_OR of all masks in
            /// ideal_affinitzation_mask_vec
            ...
            for (int affinitization_attempts = 0; affinitization_attempts < 2; ++affinitization_attempts) {
                ...
                for (unsigned int gpu_idx = 0; gpu_idx < num_gpu; ++gpu_idx) {
                    ...
                    for (int cpu_idx = 0; cpu_idx != num_cpu && gpu_cpu_count < num_cpu_per_gpu; ++cpu_idx) {
                        if (CPU_ISSET(cpu_idx, ideal_affinitization_mask_vec.at(gpu_idx))) {
            ...
            if (cpu_remaining != 0) {
                throw Exception("NVMLGPUTopo::" + std::string(__func__) + ": Failed to affinitize all valid CPUs to GPUs.  "
